...
class resThread(object):
    ...
    def tempWarning(what):
        ...
        try:
            ...
            file1.write(str(data) + " : " + str(int(st.mean(resThread.cpu_temps))) + "  " + str(what) + '\n')
            file1.close()
        except:
            print("failTW")
                ...
                if resThread.flags[2] == True:
                    ...
                    if resThread.tempLimiter_CPU == True:
                        if st.mean(resThread.cpu_temps) > resThread.max_cput_allowed:
                            resThread.tempWarning("cpu")
                    if resThread.STOP_THREAD == True: break
            except:
                print("temp")

            #get gpu temp
            if resThread.flags[6] == True:
                values = []
                for sensor in resThread._sensors.Sensor():
                    if sensor.SensorType == 'Temperature':
                        if 'GPU Core' in sensor.Name:
                            resThread.gpu_temp = float(sensor.Value)
                #Verificare temperatura admisa
                if resThread.tempLimiter_GPU == True:
                    if st.mean(resThread.gpu_temp) > resThread.max_gput_allowed:
                        resThread.tempWarning("gpu")
                    if resThread.STOP_THREAD == True: break

            #get gpu usage
            if resThread.flags[5] == True:
                values = []
                for sensor in resThread._sensors.Sensor():
                    if sensor.SensorType == 'Load':
                        if 'GPU Core' in sensor.Name:
                            resThread.gpu_freq = int(sensor.Value)
                    if resThread.STOP_THREAD == True: break
